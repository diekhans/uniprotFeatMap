...
class runIfNotDone:
    ...
    see if something is done. The with context returns a True if you need to
    do something, false otherwise.  Checks if target exists. If there are
    dependencies, the check that target.done is newer.  depends are check
    for actually files, doneDepends are check for done files
